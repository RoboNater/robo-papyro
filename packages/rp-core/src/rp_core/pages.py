...
class PageSpecError(InputError, ValueError):
...
def contiguous_runs(numbers: Sequence[int]) -> list[tuple[int, int]]:
    """Group a sorted list of page numbers into inclusive contiguous (start, end)
    runs, so a tool that takes a first/last page range can be invoked once per
    run instead of once per page."""
